...
class PlanningAgent:
    ...
    async def execute_planning_workflow(
        ...
        if not parse_result.get('success'):
            return self._create_error_result(
                "parse_failed",
                parse_result.get('error', 'Unknown parsing error')
            )
        
        plan_data = parse_result['data']
        self._log_step("parse_complete", {
        ...
        optimization_result = await self.tools.optimize_schedule(
            plan_data,
            optimization_goals
        )
        ...
        if not optimization_result.get('success'):
            ...
            self._log_step("optimization_complete", {
    ...
    async def _generate_assumptions(
        ...
        tasks_with_deps = [
            t for t in plan_data.get('tasks', [])
    ...
    async def _generate_recommendations(
        ...
        if self.ai_service:
            try:
                ai_recommendations = await self.ai_service.suggest_improvements(
                    plan_data,
                    metrics
                )
                return [
